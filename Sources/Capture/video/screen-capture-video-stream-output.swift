...
internal final class ScreenCaptureVideoStreamOutput: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let systemAudioWriter: ScreenCaptureSystemAudioWriter?
    ...
    init(
        ...
        systemAudioWriter: ScreenCaptureSystemAudioWriter? = nil,
    ...
    func stream(
        ...
        writer.fail(
            CaptureError.videoCapture(
                Self.describeVideoError(
                    error
                )
            )
        )

        systemAudioWriter?.fail(
            CaptureError.audioCapture(
                Self.describeAudioError(
                    error
                )
            )
        )
...
internal extension ScreenCaptureVideoStreamOutput {
    ...
    static func describeVideoError(
        _ error: Error
    ) -> String {
        let nsError = error as NSError

        return "Screen video stream stopped unexpectedly. \(nsError.localizedDescription) domain=\(nsError.domain) code=\(nsError.code) userInfo=\(nsError.userInfo)"
    }

    static func describeAudioError(
        _ error: Error
    ) -> String {
        let nsError = error as NSError
